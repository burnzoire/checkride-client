...
describe('Slapshot — evaluate', () => {
  ...
  it('returns false when weaponClass is not ARM', () => {
    const state = stateWithSamLaunch(102000);
    expect(slapshot.evaluate(shotEvent({ weaponClass: 'AAM' }), state)).toBe(false);
  });

  it('returns false when weaponClass is a generic missile (not ARM-classified)', () => {
    const state = stateWithSamLaunch(102000);
    expect(slapshot.evaluate(shotEvent({ weaponClass: 'MISSILE' }), state)).toBe(false);
  });

  it('returns false when weaponClass is missing', () => {
    const state = stateWithSamLaunch(102000);
    expect(slapshot.evaluate(shotEvent({ weaponClass: undefined }), state)).toBe(false);
  });
